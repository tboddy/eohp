...
static void wriggleTwo(s16 i){
	...
	if(enemies[i].pos.y > FIX16(GAME_H / 2 + 1)){
		...
		if(enemies[i].clock % 240 < 80 && enemies[i].clock % 15 == 0){
			if(enemies[i].clock % 240 == 0) enemies[i].ints[2] = 256 + 32 + random() % 32;
			struct bulletSpawner spawner = {
			...
			void updater(j){
				if(bullets[j].clock >= 60){
					if(bullets[j].clock == 60){
						bullets[j].angle = 256 + random() % 512;
					}
			...
			spawner.angle += 48;
			for(u8 j = 0; j < 4; j++){
				spawner.x = fix16Add(enemies[i].pos.x, fix16Mul(cosFix16(spawner.angle), FIX16(28 * (enemies[i].clock % 120 / 15 + 2))));
				spawner.y = fix16Add(enemies[i].pos.y, fix16Mul(sinFix16(spawner.angle), FIX16(28 * (enemies[i].clock % 120 / 15 + 2))));
				spawnBullet(spawner, updater);
				spawner.angle += 96;
			}
		}
